...
volcanoPlot=function(foldchange,pvalue,foldchange_threshold=2,pval_threshold=0.05,log2FC=T)
{
  ...
  color=rep(NA,length(foldchange))
  color[interestingPval]=paste0("pval<",pval_threshold)
  color[interestingStat]=paste0("|F|>",foldchange_threshold)
  color[interestingPval&interestingStat]=paste0("|F|>",foldchange_threshold,"and","pval<",pval_threshold)
  df=data.frame(lab=names(foldchange),log2FoldChange=log2FoldChange,log10p=log10p,color=color)
  gg= ggplot(df) +
    geom_point(aes(x=log2FoldChange, y=log10p, colour=color)) +
  ...
  gg=gg+geom_text(data=df[interestingPval|interestingStat,],aes(x=log2FoldChange,y=log10p,label=lab,colour=color))
